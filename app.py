...
async def api(path: str):
    ...
    data = await redis.zincrby(URL_PATH_COUNT_DATA_STRUCTURE, 1, path)
...
async def stats():
    ...
    response = await redis.zrevrange(
        name=URL_PATH_COUNT_DATA_STRUCTURE,
        start=0,
        end=-1,
        withscores=True,
